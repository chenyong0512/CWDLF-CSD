...
# 2. 执行 GMM 聚类
# ==========================
def run_gmm(data, n_clusters=3, random_state=42):
    """
    对二维数据执行高斯混合模型 (GMM) 聚类
...
# 4. 可视化结果
# ==========================
def plot_clusters(data, labels, centers, title="GMM 2D Clustering"):
...
labels, centers = run_gmm(data, n_clusters=n_clusters)
evaluate_clustering(data, labels)
plot_clusters(data, labels, centers, title=f"GMM Clustering (k={n_clusters})")
